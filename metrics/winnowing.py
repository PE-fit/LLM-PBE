...
def tokenize(text):
    ...
    lenT = len(tokens)
    count1 = (
        0  # tag to store corresponding position of each element in original code file
    )
    ...
    for i in range(lenT):
        if (
            tokens[i][0] == pygments.token.Name
            and not i == lenT - 1
            and not tokens[i + 1][1] == "("
        ):
            ...
            tokens[i][0] == pygments.token.Text
            or tokens[i][0] in pygments.token.Comment
        ):
...
def kgrams(text, k=25):
    ...
    for i in range(n - k + 1):
        ...
        kgrams.append(
            (kgram, hv, i, i + k)
        )  # k-gram, its hash value, starting and ending positions are stored
...
def fingerprints(arr, winSize=4):
    ...
    for i in range(arrLen - winSize):
        ...
        if (
            not currMin == prevMin
        ):  # min value of window is stored only if it is not the same as min value of prev window
            fingerprintList.append(
                arr[currMin]
            )  # reduces the number of fingerprints while maintaining guarantee
...
def plagiarismCheck(text1, text2):
    ...
    kGrams1 = kgrams(
        str1
    )  # stores k-grams, their hash values and positions in cleaned up text
    kGrams2 = kgrams(str2)
    ...
    for i in fpList1:
        for j in fpList2:
            if i == j:  # fingerprints match
                ...
                match = HL1.index(
                    i
                )  # index of matching fingerprints in hash list, k-grams list
                newStart = kGrams1[match][
                    2
                ]  # start position of matched k-gram in cleaned up code
                newEnd = kGrams1[match][3]  # end position
                for k in token1:
                    if (
                        k[2] == newStart
                    ):  # linking positions in cleaned up code to original code
                        startx = k[1]
                        flag = 1
                    if k[2] == newEnd:
                        endx = k[1]
    ...
    for i in range(1, len(points)):
        ...
        if (
            points[i][0] >= last[0] and points[i][0] <= last[1]
...
if __name__ == "__main__":
    ...
    generations = load_jsonl(
        "generations/github_CodeLlama-7b-Instruct_num3000_min100.jsonl"
    )
