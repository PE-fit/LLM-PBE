...
def more_args():
    ...
    parser.add_argument(
        ...
        default=2000,
        type=int,
        help="Total number of updates to perform.",
    )
    parser.add_argument(
        "--do_unlearn", action="store_true", help="Whether to run unlearning."
    )

    parser.add_argument(
        "--forget_file", default=None, type=str, help="The input forget set."
    )
    parser.add_argument("--new_file", default=None, type=str, help="The input new set.")

    parser.add_argument(
        "--retain_loss_ratio", default=0.1, type=float, help="Ratio for remaining loss."
    )
...
def unlearn(
    ...
    train_dataloader = get_dataLoader(
        args, train_dataset, model, tokenizer, shuffle=True
    )
    forget_dataloader = get_dataLoader(
        args, forget_dataset, model, tokenizer, shuffle=True
    )
    ...
    logger.info(
        f"Train: {len(train_dataset)}, Forget: {len(forget_dataset)}, New: {len(new_dataset)}, Dev: {len(dev_dataset)}"
    )
    logger.info(
        f"Num Updates/Epochs - {args.num_train_updates}/{args.num_train_epochs}"
    )
    ...
    while epoch_num < args.num_train_epochs and not stop:
        model.train()
        for total_steps, batch_data in enumerate(
            tqdm(train_dataloader, desc=f"Training epoch {epoch_num}"), start=1
        ):
            batch_data_model = {k: v.to(model.device) for k, v in batch_data.items()}
            batch_data_train_model = {
                k: v.to(train_model.device) for k, v in batch_data.items()
            }

            outputs = model(**batch_data_model)  # A_*, the model that trained
            tgt_outputs = train_model(
                **batch_data_train_model
            )  # A_D, the model that before unlearned

            pred_logits = F.log_softmax(outputs.logits, dim=-1).to(model.device)
            tgt_logits = (
                F.log_softmax(tgt_outputs.logits, dim=-1).detach().to(model.device)
            )

            loss_remain = F.kl_div(
                input=pred_logits,
                target=tgt_logits,
                log_target=True,
                reduction="batchmean",
            )  # Computer Lr based on Eq. 4
            ...
            accelerator.backward(
                loss_remain * args.retain_loss_ratio / args.update_freq
            )
            ...
            if total_steps % args.update_freq == 0:
                ...
                batch_forget_model = {
                    k: v.to(model.device) for k, v in batch_forget_model.items()
                }
                batch_forget_forget_model = {
                    k: v.to(forget_model.device) for k, v in batch_forget_model.items()
                }
                batch_new_train_model = {
                    k: v.to(train_model.device) for k, v in batch_new.items()
                }
                batch_new_new_model = {
                    k: v.to(new_model.device) for k, v in batch_new.items()
                }
                ...
                tgt_logits = (
                    F.log_softmax(tgt_outputs.logits, dim=-1)
                    .detach()
                    .to(train_model.device)
                )
                tgt_align = F.kl_div(
                ...
                loss_align = torch.abs(
                    loss_align - tgt_align.item()
                )  # Compute La based on Eq. 3
                ...
                if stop_value is None and args.stop_portion is not None:
                    ...
                    logger.info(
                        f"Set stop value as {stop_value} (Portion {args.stop_portion})"
                    )
                ...
                total_loss = (
                    loss_align.item() + args.retain_loss_ratio * loss_remain.item()
                )  # La + a * Lr

                if total_updates % 20 == 0:
                    logger.info(
                        f"Train Step {total_updates}, total loss: {total_loss:>7f}, align loss {loss_align.item():>7f}, retain loss {loss_remain.item():>7f}"
                    )
        ...
        forget_loss = dev_loop(
            args,
            get_dataLoader(args, forget_dataset, model, tokenizer, shuffle=False),
            model,
        )
        logger.info(f"Loss (forget) {forget_loss:0.4f}")

        logger.info(
            f"Epoch {epoch_num} finished, saving to epoch_{epoch_num}_update_{total_updates}_dev_loss_{dev_loss:0.4f}_forget_loss_{forget_loss:0.4f}_weights.bin"
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_dir):
        ...
        if any([(".bin" in f) for f in files]):
            raise ValueError(
                f"Output directory ({args.output_dir}) already exists saved models."
            )
    ...
    train_model = AutoModelForCausalLM.from_pretrained(
        args.train_model_dir, trust_remote_code=True
    )
    ...
    if args.do_unlearn:
        ...
        raw_datasets = EchrDataset(
            data_path="/home/junyi/unlearn/LLM-PBE/data/echr",
            pseudonymize=True,
            mode="undefended",
        ).raw_datasets  # 没有做任何防御，原始数据

        forget_ids = load_ids(args.forget_file)
        new_ids = load_ids(args.new_file)
        train_ids = list(
            set(range(len(raw_datasets["train"]))) - set(forget_ids) - set(new_ids)
        )  # exclude forget and new

        train_dataset = raw_datasets["train"].select(
            train_ids
