...
def train_loop(
    args, dataloader, model, optimizer, lr_scheduler, epoch, total_loss, total_updates
):
    ...
    for step, batch_data in enumerate(dataloader, start=1):
        ...
        if step % 1000 == 0:
            logger.info(
                f"Train Epoch {epoch+1} Step {step}/{len(dataloader)}: Loss {total_loss/(finish_step_num + step):>7f}"
            )
...
def train(args, train_dataset, dev_dataset, model, tokenizer):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        logger.info(
            f"Train Epoch {epoch+1} End! Loss {total_loss / ((epoch+1) * len(train_dataloader)):>7f}"
        )
        ...
        if dev_loss < best_dev:
            ...
            if saved is not None and os.path.exists(
                os.path.join(args.output_dir, saved)
            ):
...
def metric_evaluate(args, dataset, model, tokenizer):
    ...
    with torch.no_grad():
        for batch_data in dataloader:
            ...
            labels += [
                [word_tokenize(label.strip().lower())] for label in decoded_labels
            ]
...
def diff_evaluate(args, dataset, base_model, unlearn_model, tokenizer):
    dataloader = get_dataLoader(
        args, dataset, model, tokenizer, batch_size=1, shuffle=False
    )
    logger.info("***** Running evaluation *****")
...
if __name__ == "__main__":
    ...
    # Testing
        ...
        if os.path.isdir(args.output_dir):
            ...
            model.load_state_dict(
                torch.load(os.path.join(args.output_dir, save_weights[-1]))
            )
    ...
    # Evaluating unlearning effects
        ...
        if os.path.isdir(args.unlearn_model_dir):
            save_weights = [
                file
                for file in os.listdir(args.unlearn_model_dir)
                if file.endswith(".bin")
            ]
            model.load_state_dict(
                torch.load(os.path.join(args.unlearn_model_dir, save_weights[-1]))
            )
        else:
