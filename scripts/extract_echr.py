...
output_fname = os.path.join(
    folder,
    f"{model_card}_{args.data_name}_num{args.num_sample}_max{args.max_token}.jsonl",
)
result = []
...
for i, indx in enumerate(subsample_indx):
    ...
    prompt = encoding.decode(
        encoding.encode(context)[-args.max_token :]
    )  # only use the last args.max_token tokens
