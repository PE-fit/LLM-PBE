...
class EnronDataExtraction:
    def __init__(self, data_path="path_to_enron_data"):
        ...
        self.few_shot_known_domain = load_jsonl(
            os.path.join(data_path, "five_shot.jsonl")
        )
        self.few_shot_unknown_domain = load_jsonl(
            os.path.join(data_path, "five_shot_non_domain.jsonl")
        )
    ...
    def get_prompts_few_shot(self, k_shot=0, domain_known=False, pp="d"):
        ...
        if k_shot == 0:
            for item in self.few_shot_known_domain:
                ...
                prompt = (
                    domain_template.format(domain) + zero_shot_template.format(name)
                    if domain_known == True
                    else zero_shot_template.format(name)
                )
                prompts.append(prompt)
                labels.append(email)
        elif k_shot > 0:
            few_shot_dict = (
                self.few_shot_known_domain
                if domain_known == True
                else self.few_shot_unknown_domain
            )
        ...
        if "prefix" in format:
            ...
            for item in self.context:
                ...
                prompt = tokenizer.decode(
                    tokenizer(context[-1024:])["input_ids"][-length:]
                )
                prompts.append(prompt)
            ...
            return self.get_prompts_few_shot(
                k_shot=k_shot, domain_known=True, pp=template
            )
        elif "-shot-unknown-domain-" in format:
            template = format.split("-")[-1]
            k_shot = int(format.split("-")[0])
            return self.get_prompts_few_shot(
                k_shot=k_shot, domain_known=False, pp=template
            )
