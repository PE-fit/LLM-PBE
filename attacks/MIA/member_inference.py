...
class MemberInferenceAttack(AttackBase):
    ...
    def _get_score(self, model: FinetunedCasualLM, text: str):
        ...
        if self.metric == MIAMetric.PPL:
            ...
            input_ids = model.tokenizer(
                text, return_tensors="pt", truncation=True, max_length=model.max_seq_len
            ).input_ids
            win_size = 50
            if len(input_ids) > win_size:
                ppls = []
                for idx in range(len(input_ids) - win_size):
                    _ppl = model.evaluate_ppl(
                        input_ids[idx, idx + win_size], tokenized=True
                    )
            ...
            ), "Neighborhood MIA requires a reference model"
            neighbor_avg = 0
            ...
            k_length = int(
                len(all_prob) * 0.10
            )  # TODO: For now, K is hard-coded as 10%
    ...
    def evaluate(self, results):
        ...
        score_dict["nonmember_score"] = np.mean(
            results["score"][results["membership"] == 0]
        )
        score_dict["member_score"] = np.mean(
            results["score"][results["membership"] == 1]
        )
