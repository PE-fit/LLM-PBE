...
parser.add_argument(
    ...
    default=None,
    type=int,
    help="whether to seed the selection of test set.",
)
...
if args.num_sample > 0 and args.num_sample < len(test_set):
    if args.seed_test_set is None:
        ...
        idxs = np.random.RandomState(args.seed_test_set).choice(
            len(test_set), args.num_sample, replace=False
        )
    test_set = test_set.select(idxs)
if args.peft == "none":
    llm = FinetunedCasualLM(
        model_path=args.model, arch=args.arch, max_seq_len=args.max_seq_len
    )
else:
...
if metric in (MIAMetric.REFER, MIAMetric.LIRA, MIAMetric.NEIGHBOR):
    ref_llm = FinetunedCasualLM(
        model_path=args.arch, arch=args.arch, max_seq_len=args.max_seq_len
    )
    ref_llm._lm.eval()
else:
    ref_llm = None
...
attack = MemberInferenceAttack(
    metric=metric, ref_model=ref_llm, n_neighbor=args.n_neighbor
)
