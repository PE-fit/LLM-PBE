...
def generate_neighbors(
    text, search_tokenizer, search_model, search_embedder, p=0.7, k=5, n=50
):
    """
...
def get_neighborhood_score(
    ...
    original_score = get_loss(
        text, label, target_tokenizer, target_model, max_length, causal=causal
    )

    # Compute log likelihood for each neighbor in the neighborhood
    neighbor_scores = []
    neighbors = generate_neighbors(
        text, search_tokenizer, search_model, search_embedder
    )
    for n in neighbors:
        neighbor_scores.append(
            get_loss(
                n, label, target_tokenizer, target_model, max_length, causal=causal
            )
        )
