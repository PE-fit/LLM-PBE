...
class CustomECHR(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        self.df = concatenate_datasets(
            [self.df["train"], self.df["test"], self.df["validation"]]
        )
        ...
        if self.config.shuffle_facts_seed > 0:
            self.data = [
                self.data[i]
                for i in rnd_idx(N=len(self.data), seed=self.config.shuffle_facts_seed)
            ]
