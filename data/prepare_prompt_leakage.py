...
def extract_general_prompts(english_only=True):
    ...
    def extract(content: str):
        # Using regular expression to extract content between specific markers
        matches = re.findall(
            r"# Prompt\s+```(.*?)```\s+## Conversation", content, re.DOTALL
        )
        return matches[0]
    ...
    for cat in categories:
        ...
        file2prompts = read_md_files(
            root_path,
            extract,
            files=[
                f for f in md_filepaths if os.path.exists(os.path.join(root_path, f))
            ],
        )
...
def is_mostly_english(input_string):
    ...
    non_english_chars = re.findall(
        r"[^a-zA-Z\s]", input_string
...
def deduplicate_prompts(prompts):
    ...
    for i in tqdm(range(len(prompts))):
        ...
        for j in range(i + 1, len(prompts)):
            if abs(len(prompts[i]) - len(prompts[j])) > min(
                len(prompts[i]) * 0.05, len(prompts[j]) * 0.05
            ):
                continue
...
if __name__ == "__main__":
    ...
    for cat in categories:
        print(f"== {cat} ==")
        merge_and_deduplicate_prompts(
            files=[os.path.join("blackfriday", f"{cat}.pth")],
            out_file=os.path.join("blackfriday", f"dedup_{cat}.pth"),
        )
