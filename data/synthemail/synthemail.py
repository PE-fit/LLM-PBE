...
from llm_pft.utils.output import print_dict_highlighted, print_highlighted
...
class CustomSynthEmail(datasets.GeneratorBasedBuilder):
    ...
    def _info(self):
        ...
        if self.config.pseudonymize:
            fea_dict.update(
                {
                    entity_class: datasets.Value("string")
                    for entity_class in self._tagger.get_entity_classes()
                }
            )
        features = datasets.Features(fea_dict)
        return datasets.DatasetInfo(description=self._DESCRIPTION, features=features)
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.shuffle_facts_seed > 0:
            shuffled_indices = rnd_idx(
                N=len(self.data), seed=self.config.shuffle_facts_seed
            )
    ...
    def _generate_examples(self, split: str, start: float, end: float):
        ...
        for i, text in enumerate(self.data[start_pos:end_pos]):
            if self.config.pseudonymize:
                ...
                pii_annotations = {
                    k: ListPII() for k in self._tagger.get_entity_classes()
                }
                pii_annotations.update(
                    {k: v.dumps() for k, v in piis.group_by_class().items()}
                )
            else:
